...
const Discover = (props: Props) => {
    ...
    useEffect(() => {
        const getShows = async () => {
            if (token) {
                ...
                try {
                    const response = await fetch('https://api.spotify.com/v1/browse/categories', artistParamter);
                    const data = await response.json();
                    setClasses(data.categories.items);
                } catch (error) {
                    console.log(error);
                }
            } else {
    ...
    return (
        <div className="flex flex-col flex-grow ml-2 w-[750px] z-10 justify-start">
            <h1 className="text-white text-2xl font-bold p-2"> Discover </h1>
            <div className='flex grid grid-cols-5 gap-2 ml-6'>
                {
                    classes && classes.map((categorie : any) => (
                        <motion.div key={categorie.id} initial={{opacity: 0, y: -20}} animate={{opacity: 1, y: 0}} exit={{opacity: 0}}>
                            <Image src={categorie.icons[0].url} title='icon' className='h-[80px] rounded-xl' alt='icon' height={80} width={80}/>
